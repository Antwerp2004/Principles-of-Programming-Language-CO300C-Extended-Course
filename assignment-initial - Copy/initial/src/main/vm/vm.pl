...
map_args_params([ArgValue|ArgRest], [par(ParamName, ParamType)|ParamRest], [id(ParamName, par, ParamType, ArgValue)|ScopeRest], CallTerm) :-
    get_type(ArgValue, ArgType),
    % Strict type check for argument passing
    (ArgType == ParamType -> true ; throw(type_mismatch(CallTerm))),
    map_args_params(ArgRest, ParamRest, ScopeRest, CallTerm).
...
% Call statement
reduce_stmt(config(call(Name, ArgsExprList), EnvIn), config(normal, FinalEnv)) :-
...
% Define lists of expected types for built-ins
builtin_expected_arg_types(writeInt, [integer]).
builtin_expected_arg_types(writeIntLn, [integer]).
builtin_expected_arg_types(writeReal, [float]).
builtin_expected_arg_types(writeRealLn, [float]).
builtin_expected_arg_types(writeBool, [boolean]).
builtin_expected_arg_types(writeBoolLn, [boolean]).
builtin_expected_arg_types(writeStr, [string]).
builtin_expected_arg_types(writeStrLn, [string]).
builtin_expected_arg_types(writeLn, []).


check_builtin_arg_types([], [], _).
check_builtin_arg_types([ArgValue|RestArgValues], [ExpectedType|RestExpectedTypes], CallTerm) :-
    get_type(ArgValue, ActualType),
    (ActualType == ExpectedType -> true ; throw(type_mismatch(CallTerm))),
    check_builtin_arg_types(RestArgValues, RestExpectedTypes, CallTerm).
check_builtin_arg_types(_, [], _) :- !, fail.
check_builtin_arg_types([], _, _) :- !, fail.
	...
	;
		...
		((is_builtin(Name, proc) ->
			...
			(ActualArity == ExpectedArity ->
				...
				reduce_args(ArgsExprList, EnvIn, ArgValues),
				(builtin_expected_arg_types(Name, ExpectedTypesList) ->
                    check_builtin_arg_types(ArgValues, ExpectedTypesList, CallTerm)
                ; true
                ),
